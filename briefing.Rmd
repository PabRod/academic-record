...
secs <- select(secs, Date, End.date, Institution)
...
```{r Conferences}
confs <- filter(data, (Type == 'Congress' | Type == 'Workshop') & Role == 'Attendant')
confs <- select(confs, Date, Name, Institution, Country, Language)
...
talks <- select(talks, Date, Name, Type, Institution, Language)
kable(talks, format='markdown')
```

## Teaching
```{r Classes}
classes <- filter(data, Type == 'Course' & (Role == 'Speaker' | Role == 'Teaching assistant'))
classes <- select(classes, Date, End.date, Name, Institution, Language)
...
Up to date, I've published `r nrow(sci_comm)` science communication essays: `r nrow(sci_comm_table_en)` in english, and `r nrow(sci_comm_table_es)` in spanish language.
...
In green you can see the countries I've visited for academic reasons:
