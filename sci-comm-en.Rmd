...
Up to date, I've given `r nrow(talks)` talks for a general audience. Additionally, I've published `r nrow(sci_comm)` science communication essays: `r nrow(sci_comm_table_en)` in English, and `r nrow(sci_comm_table_es)` in Spanish.

### Talks
```{r TalksTab}
kable(talks, format='pandoc')
```

### Science communication publications in English

```{r ScicommEnglish}
kable(sci_comm_table_en, format='pandoc')
```

### Science communication publications in Spanish

```{r ScicommTab}
kable(sci_comm_table_es, format='pandoc')
```

### Others
```{r Others tab}
kable(other_table, format='pandoc')
```
